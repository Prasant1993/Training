...
def load_file(input_file, selection = None):
    ...
    for varname in barrel_vars + ["pt"]:
        ...
        for tree_name, label in [
            ...
            if varname == "pt":
                pt_values.append(tree[varname].array(library="np")[indices])
            elif varname == "scEta":
                scEta_values.append(tree[varname].array(library="np")[indices])
            ...
            if is_first_var:
                ...
                if label == 1:
                    ...
                    this_weights = this_weights * tree["PtvsEtaWeight"].array(library="np")[indices]
