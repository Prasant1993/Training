  ...
  */
  ...
  for(int i = 0; i < 2; i++){
    ...
    for(int k = 0; k < nCuts; k++){
      ...
      std::fstream outfile;
      if(i==0){
        outfile.open("sigEff_bkgEff_endcap_Rereco16_TMVA_SATrain_UL16_SATest_20000_nbins2000.txt",std::fstream::out | std::fstream::app);
        outfile << bkgEff[k] <<'\t'<< sigEff[k] <<'\t'<< bkgrejEff[k] <<'\t'<<  mvaResCutVal << endl;
	} 

      if(i==1){
        outfile.open("sigEff_bkgEff_endcap_UL16_TMVA_SATrain_UL16_SATest_20000_nbins2000.txt",std::fstream::out | std::fstream::app);
        outfile << bkgEff[k] <<'\t'<< sigEff[k] <<'\t'<< bkgrejEff[k] <<'\t'<<  mvaResCutVal << endl;
	}
  ...
  string mvaRes = "";
  mvaRes = "mvares_EE_phoId_Hgg_UL16_SATest_20000_nbins_2000_V2";
  ...
  canName_RoC = "RoC_EE_phoId_Hgg_UL16_train_UL16_SATest_20000_nbins_2000_V2";
