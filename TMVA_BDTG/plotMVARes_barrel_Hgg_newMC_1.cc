  ...
  */
  ...
  for(int i = 0; i < 3; i++){
    ...
    for(int k = 0; k < nCuts; k++){
      ...
      /* std::fstream outfile;
      if(i==0){
	outfile.open("sigEff_bkgEff_barrel_UL18_TMVA_SATrain_UL18_SATest_20000_nbins2000.txt",std::fstream::out | std::fstream::app);
	outfile << bkgEff[k] <<'\t'<< sigEff[k] <<'\t'<< bkgrejEff[k] <<'\t'<<  mvaResCutVal << endl;
	} 
      if(i==1){
        outfile.open("sigEff_bkgEff_barrel_UL18_XGB_SATrain_UL18_SATest_20000_nbins2000.txt",std::fstream::out | std::fstream::app);
        outfile << bkgEff[k] <<'\t'<< sigEff[k] <<'\t'<< bkgrejEff[k] <<'\t'<<  mvaResCutVal << endl;
	}
      if(i==2){
  ...
  string mvaRes = "";
  mvaRes = "mvares_EB_phoId_UL18_SATest_20000_nbins_2000_V1";
  ...
  canName_RoC = "RoC_EB_phoId_UL18_SATest_20000_nbins_2000_0p8range_V2";
