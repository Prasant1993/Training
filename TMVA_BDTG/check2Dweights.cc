...
void plotAnyVariable(TCanvas * can, TCut baseCut, TCut twoDwCut, string varName, TLegend * legend, string label, int nbins, float min, float max, double maxHisto){
  TFile *File = TFile::Open("Output_SinglePhoton_GJet_Train.root");
...
void check2Dweights(){
  ...
  if(doEE) tmp = "_94X_EEPho_Hgg_MCv2";
  if(doEB) tmp = "_94X_EBPho_Hgg_MCv2";
  ...
  if(plot_pT){
    ...
    TCut baseCut_rho = "(covIphiIphi<100 && pt > 18)*weight";
    TCut twoDwCut_rho = "(covIphiIphi<100 && pt > 18)*weight*PtvsEtaWeight";

    if(doEB) {
      baseCut_rho = "(abs(scEta)<1.4442 && pt > 18)*weight";
      twoDwCut_rho = "(abs(scEta)<1.4442 && pt > 18)*weight*PtvsEtaWeight";
    }

    if(doEE) {
      baseCut_rho = "(abs(scEta)>1.566 && pt > 18)*weight";
      twoDwCut_rho = "(abs(scEta)>1.566 && pt > 18)*weight*PtvsEtaWeight";
    }
  ...
  if(plot_SCEta){
    ...
    TCut baseCut_SCEta = "(covIphiIphi<100 && pt > 18)*weight";
    TCut twoDwCut_SCEta = "(covIphiIphi<100 && pt > 18)*weight*PtvsEtaWeight";

    if(doEB) {
      baseCut_SCEta = "(abs(scEta)<1.4442 && pt > 18)*weight";
      twoDwCut_SCEta = "(abs(scEta)<1.4442 && pt > 18)*weight*PtvsEtaWeight";
    }

    if(doEE) {
      baseCut_SCEta = "(abs(scEta)>1.566 && pt > 18)*weight";
      twoDwCut_SCEta = "(abs(scEta)>1.566 && pt > 18)*weight*PtvsEtaWeight";
    }
