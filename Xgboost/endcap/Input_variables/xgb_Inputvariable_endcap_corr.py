...
fin_EE = uproot.open("/home/prasant/Files/Lowmass_ntuple/Out_Singlephoton_EE_Lowmass_photonIDMVA_woShowershape_LMTrain_18pT18_RunIIFall17_3_1_0_03122018.root");
...
tree_p = fin_EE['promptPhotons']
tree_f = fin_EE['fakePhotons']

names = ['SCRawE', 'r9', 'sigmaIetaIeta', 'etaWidth', 'phiWidth', 'covIEtaIPhi', 's4', 'phoIso03', 'chgIsoWrtChosenVtx',
               'chgIsoWrtWorstVtx', 'scEta', 'rho', 'esEffSigmaRR', 'esEnergyOverRawE']
...
geometry_selection_p = lambda prompt: np.logical_and(abs(prompt.array('scEta')) > 1.566, abs(prompt.array('scEta')) < 2.5)
geometry_selection_f = lambda fake: np.logical_and(abs(fake.array('scEta')) > 1.566, abs(fake.array('scEta')) < 2.5)
...
n_bins_esEffSigmaRR = np.linspace(0, 15, 100)

n_bins_esEnergyOverRawE = np.linspace(0, 0.5, 100)
...
def plot_Inputvar(input_file, selection_p, selection_f):
    ...
    esEffSigmaRR_p.append(prompt.array('esEffSigmaRR')[indices_p])
    print 'esEffSigmaRR_p =', esEffSigmaRR_p

    esEnergyOverRawE_p.append(prompt.array('esEnergyOverRawE')[indices_p])
    print 'esEnergyOverRawE_p =', esEnergyOverRawE_p
    ...
    esEffSigmaRR_f.append(fake.array('esEffSigmaRR')[indices_f])
    print 'esEffSigmaRR_f =', esEffSigmaRR_f

    esEnergyOverRawE_f.append(fake.array('esEnergyOverRawE')[indices_f])
    print 'esEnergyOverRawE_f =', esEnergyOverRawE_f
    ...
    plt_M.figure()
    plt_M.hist(esEffSigmaRR_p, n_bins_esEffSigmaRR, weights=original_weights_p, histtype='step', color='r', label='prompt')
    plt_M.hist(esEffSigmaRR_f, n_bins_esEffSigmaRR, weights=original_weights_f, histtype='step', color='b', label='fake')
    plt_M.yscale('log')
    plt_M.grid(True)
    plt_M.legend(loc='upper right')
    plt_M.xlabel("esEffSigmaRR")
    plt_M.savefig('esEffSigmaRR.png')

    import matplotlib.pyplot as plt_N
    plt_N.figure()
    plt_N.hist(esEnergyOverRawE_p, n_bins_esEnergyOverRawE, weights=original_weights_p, histtype='step', color='r', label='prompt')
    plt_N.hist(esEnergyOverRawE_f, n_bins_esEnergyOverRawE, weights=original_weights_f, histtype='step', color='b', label='fake')
    plt_N.yscale('log')
    plt_N.grid(True)
    plt_N.legend(loc='upper right')
    plt_N.xlabel("esEnergyOverRawE")
    plt_N.savefig('esEnergyOverRawE.png')
