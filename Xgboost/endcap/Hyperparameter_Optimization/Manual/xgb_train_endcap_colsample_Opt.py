...
for n in colsample:
    model = XGBClassifier(max_depth=6,n_estimators=260,min_child_weight=1,gamma=0.0,subsample=1,colsample_bytree=n,learning_rate=0.1,
                          n_jobs=40)
    model.fit(X_train, y_train, sample_weight = w_train)
...
plt_A.figure()
plt_A.plot(colsample, train_results_A, 'b', label='Train AUC')
plt_A.plot(colsample, test_results_A, 'r', label='Test AUC')
plt_A.legend(loc='lower right')
plt_A.ylabel('AUC Score')
plt_A.xlabel('colsample_bytree')
plt_A.title('AUC Score vs colsample_bytree')
plt_A.grid()
plt_A.savefig('colsample_bytree_optimization_endcap_16032019_predict.png')
...
plt_B.figure()
plt_B.plot(colsample, train_results_B, 'b', label='Train AUC')
plt_B.plot(colsample, test_results_B, 'r', label='Test AUC')
plt_B.legend(loc='lower right')
plt_B.ylabel('AUC Score')
plt_B.xlabel('colsample_bytree')
plt_B.title('AUC Score vs colsample_bytree')
plt_B.grid()
plt_B.savefig('colsample_bytree_optimization_endcap_16032019_predict_proba.png')
