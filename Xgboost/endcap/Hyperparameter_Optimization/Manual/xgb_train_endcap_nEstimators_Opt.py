...
nestimators = [1, 10, 20, 30, 50, 70, 90, 100, 150, 200, 300, 400, 500]

for n in nestimators:
    model = XGBClassifier(max_depth=6,n_estimators=n,learning_rate=0.1,n_jobs=40)
    model.fit(X_train, y_train, sample_weight = w_train)
...
plt_A.figure()
plt_A.plot(nestimators, train_results_A, 'b', label='Train AUC')
plt_A.plot(nestimators, test_results_A, 'r', label='Test AUC')
plt_A.legend(loc='lower right')
plt_A.ylabel('AUC Score')
plt_A.xlabel('n_estimators')
plt_A.title('AUC Score vs n_estimators')
plt_A.grid()
plt_A.savefig('n_estimators_optimization_endcap_16032019_predict.png')
...
plt_B.figure()
plt_B.plot(nestimators, train_results_B, 'b', label='Train AUC')
plt_B.plot(nestimators, test_results_B, 'r', label='Test AUC')
plt_B.legend(loc='lower right')
plt_B.ylabel('AUC Score')
plt_B.xlabel('nestimators')
plt_B.title('AUC Score vs nestimators')
plt_B.grid()
plt_B.savefig('n_estimators_optimization_endcap_16032019_predict_proba.png')
