...
alpha = [0.0, 0.001, 0.005, 0.01, 0.05]

for n in alpha:
    model = XGBClassifier(max_depth=6,n_estimators=260,min_child_weight=1,gamma=0.0,subsample=1,colsample_bytree=1,reg_alpha=n,
                          learning_rate=0.1,n_jobs=40)
    model.fit(X_train, y_train, sample_weight = w_train)
...
plt_A.figure()
plt_A.plot(alpha, train_results_A, 'b', label='Train AUC')
plt_A.plot(alpha, test_results_A, 'r', label='Test AUC')
plt_A.legend(loc='lower right')
plt_A.ylabel('AUC Score')
plt_A.xlabel('reg_alpha')
plt_A.title('AUC Score vs reg_alpha')
plt_A.grid()
plt_A.savefig('reg_alpha_optimization_endcap_16032019_predict.png')
...
plt_B.grid()
plt_B.savefig('reg_alpha_optimization_endcap_16032019_predict_proba.png')
