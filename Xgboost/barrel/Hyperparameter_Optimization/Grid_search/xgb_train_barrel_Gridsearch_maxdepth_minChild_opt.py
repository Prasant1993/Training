...
model = XGBClassifier(learning_rate=0.1,n_estimators=459)
print(model)
...
scores = np.array(means).reshape(len(min_child_weight), len(max_depth))
for i, value in enumerate(min_child_weight):
    plt.plot(max_depth, scores[i], label='min_child_weight: ' + str(value))
plt.legend()
plt.xlabel('max_depth')
plt.ylabel('roc_auc')
plt.legend(loc = 'lower right', fontsize = 20)
plt.grid()
plt.savefig('max_depth_vs_min_child_weight_19032019.png')
