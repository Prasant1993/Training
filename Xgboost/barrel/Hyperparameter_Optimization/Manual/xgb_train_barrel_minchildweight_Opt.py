...
for n in min_child_weights:
    model = XGBClassifier(max_depth=6,n_estimators=459,min_child_weight=n,learning_rate=0.1,n_jobs=40)
...
plt_A.figure()
plt_A.plot(min_child_weights, train_results_A, 'b', label='Train AUC')
plt_A.plot(min_child_weights, test_results_A, 'r', label='Test AUC')
plt_A.legend(loc='lower right')
plt_A.ylabel('AUC Score')
plt_A.xlabel('min_child_weight')
plt_A.title('AUC Score vs min_child_weight')
plt_A.grid()
plt_A.savefig('min_child_weight_optimization_barrel_16032019_predict.png')

# plot predict_proba results

import matplotlib.pyplot as plt_B
plt_B.figure()
plt_B.plot(min_child_weights, train_results_B, 'b', label='Train AUC')
plt_B.plot(min_child_weights, test_results_B, 'r', label='Test AUC')
plt_B.legend(loc='lower right')
plt_B.ylabel('AUC Score')
plt_B.xlabel('min_child_weight')
plt_B.title('AUC Score vs min_child_weight')
plt_B.grid()
plt_B.savefig('min_child_weight_optimization_barrel_16032019_predict_proba.png')
